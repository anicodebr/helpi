...
module.exports = {
    ...
    async destroy(req,res){// Rota para destruir todos os dados relacionados a determinado usuário
        ...
        .then(user => {
            let del_cliente = false , del_admin = false, del_entregador = false;
            let _user = user.get({plain: true});//pega os dados dele pq a gnt n é burro e tu já ver
            user.destroy(); //MATO O USUÁRIO MANO MEU DEUS SOCORRO
            if(_user.ClienteId){
                Cliente.findByPk(_user.ClienteId) //caso seja cliente, procura o endereço pra diminuir espaço no banco, foda-se
                .then(cliente => {
                    ...
                    Endereco.findByPk(_cliente.EnderecoId)//Buscando endereco
                    .then(endereco => {
                        endereco.destroy() //MATO O ENDERECO MANO MEU DEUS
                        del_cliente = true // Salvo para uso futuro
                    }).catch(err => { console.error(err); return res.status(500).json(null); })//Catch para dar console no error e retornar um status de erro no servidor sem vazar dados
                }).catch(err => { console.error(err); return res.status(500).json(null); })//Catch para dar console no error e retornar um status de erro no servidor sem vazar dados
            }
            if(_user.EntregadorId){
                Entregador.findByPk(_user.EntregadorId) //caso seja entregador, deleta ele e passa pra próxima
                .then(entregador => {
                    entregador.destroy() //MATO O ENTREGADOR MANO MEU DEUS
                    del_entregador = true //Salvo para uso futuro
                }).catch(err => { console.error(err); return res.status(500).json(null); })//Catch para dar console no error e retornar um status de erro no servidor sem vazar dados
            }
            if(_user.AdminId){
                Admin.findByPk(_user.AdminId) // caso seja admin, deleta ele e passa pra saída da função
                .then(admin => {
                    admin.destroy() //MATO O ADMIN MANO MEU DEUS
                    del_admin = true //Salvo para uso futuro
                }).catch(err => { console.error(err); return res.status(500).json(null); })//Catch para dar console no error e retornar um status de erro no servidor sem vazar dados
            }
            if( del_cliente || del_admin || del_entregador ){ // caso algum deles tenha sido deletado por que é obrigatório que o usuário seja alguma das opções acima, deleta ele também e bora pra próxima
